...
Вычислить вектора осей сложнее, чем двумерном пространстве. Для примера получения этих векторов рассмотрим алгоритм, который в трехмерных движках называется lookAt. Для этого нам понадобятся вектор направления взгляда - $\vec z$ и опорный вектор для оси $Y$ - $\vec y$. Сам алгоритм:
