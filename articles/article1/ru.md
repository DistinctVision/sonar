...
Матрица $R$, состовляющая ортонормированный базис и описывающая поворот, называется матрицей поворота. 
Также матрица поворота имеет ряд полезных свойств, которые следует иметь ввиду:  
* При $R = I$, где $I$ - единичная матрица, - это матрица соотвествующему нулевому повороту (угол $\alpha = 0$), и в таком случае локальные оси совпадают с мировыми.
...
Возьмем вектор $\vec n$ - описывающий ось, вокруг которой нужно повернуть вектор $\vec v$ на угол $\alpha$. Результиру.щий вектор обозначим как $\vec{v'} = rotate(\vec n, \alpha, \vec v)$. Иллюстрируем процесс: 

![$\alpha={140^{\circ}}$](./assets/Figure_9.png)

Вектор $\vec n$ мы можем разложить сумму векторов: вектора, параллельный к вектору $\vec n$ - $\vec{v_\parallel}$, и вектора, перпендикулярному к вектору к вектору $\vec n$ - $\vec{v_{\perp}}$. $\vec v = \vec{v_\parallel} + \vec{v_{\perp}}$ 
Вектор $\vec{v_\parallel}$ - это проекция вектра $\vec v$ на вектор $\vec n$. Т.к. $\vec n$ - нормализованный вектор, то:  

$\vec{v_\parallel} = \vec{n} \cdot \vec{n}^T \vec{v}$
Та часть $\vec v$, которая принадлежит оси вращения ($\vec{v_parallel}$) не измениться во время вращения. Повернуть нам нужно только $\vec{v_{\perp}}$ в плоскости перпендикулярной к $\vec n$ на угол $\alpha$, Обозначим этот вектор как $\vec{v_{\perp rot}}$. Тогда наш искомый вектор -- $\vec{v'} = \vec{v_\parallel} + \vec{v_{\perp rot}}$
Вектор $\vec{v_{\perp}}$ можем найти следующим образом: $\vec{v_{\perp}} = \vec v - \vec{v_\parallel}$
Для того, чтобы повернуть $\vec{v_{\perp}}$, выведем оси $X$ и $Y$ в плоскости, в который будем выполнять поворот. Это должны быть два ортогональных нормализованных вектора, ортогональных к $\vec n$. Один ортогональный вектор у нас уже есть -- $\vec{v_{\perp}}$, нормализуем его и обозначим как ось $X$ -- $\vec{aX} = \frac{\vec{v_{\perp}}}{|\vec{v_{\perp}}|}$. 
Теперь получим вектор оси $Y$. Это должен быть вектор, ортогональный к $n$ и $\vec{aX}$ (т.е. и к $\vec{v_{\perp}}$). Получить его можно через векторное произведение: $\vec{d_y} = \vec n \times \vec{v_{\perp}}$. И вектор оси $\vec{aX} = \frac{\vec{d_y}}{|\vec{d_y}|}$. По свойству векторного произведения $|\vec{d_y}|$ будет равно площади параллелограмма, образуемого двумя исходными векторами ($n$ и $\vec{aX}$). Так как вектора ортогональны, то у нас будет не параллелограмм, а прямоуголник, а значит $|\vec{d_y}| = |\vec{n}| \cdot |\vec{v_{\perp}}|$. $|\vec{n}| = 1 \Rightarrow |\vec{d_y}| = |\vec{v_{\perp}}|$. Значит $\vec{aY} = \frac{\vec n \times \vec{v_{\perp}}}{|\vec{v_{\perp}}|}$.  
Поворот двумерного вектора $\vec v = \begin{pmatrix} 1 & 0 \end{pmatrix}^T$ на угол $\alpha$ можно получить через синус и косинус -- $rotate2D(\vec v, \alpha) = \begin{pmatrix} \cos{\alpha} & \sin{\alpha} \end{pmatrix}^T$. Т.к. $\vec{v_{\perp}}$ в координатах полученной плоскости сонаправлен с осью $X$, то он будет равен $\begin{pmatrix} |\vec{v_{\perp}}| & 0 \end{pmatrix}^T = \begin{pmatrix} 1 & 0 \end{pmatrix}^T \cdot |\vec{v_{\perp}}|$. Этот вектор после поврота -- $rotate2D(\begin{pmatrix} 0 & 1 \end{pmatrix}^T, \alpha) \cdot |\vec{v_{\perp}}| = \begin{pmatrix} \cos{\alpha} & \sin{\alpha} \end{pmatrix}^T \cdot |\vec{v_{\perp}}|$. Отсюда можем вывести $\vec{v_{\perp rot}} = (\vec{aX} \cdot \cos{\alpha} + \vec{aY} \cdot \sin{\alpha}) \cdot |\vec{v_{\perp}}|$  
$\vec{v_{\perp rot}} = (\frac{\vec{v_{\perp}}}{|\vec{v_{\perp}}|} \cdot \cos{\alpha} + \frac{\vec n \times \vec{v_{\perp}}}{|\vec{v_{\perp}}|} \cdot \sin{\alpha}) \cdot |\vec{v_{\perp}}|$  
...
Мы разобрались с тем, как поворачивать вектор кокруг заданной оси на заданный угол, значит теперь мы умеем использовать поворот, заданный таким образом.  
Получить вектор оси вращения и угол из вектора Родрига не состовляет большого труда, а значит мы теперь умеем работать и с ним тоже.
Напоминаю, что матрица поворота представляет собой три базисных вектора $R = \begin{pmatrix} \vec{aX} & \vec{aY} & \vec{aZ} \end{pmatrix}$, а углы Эйлера - три последовательных поворота вокруг осей $X$, $Y$, $Z$. Значит мы можем взять единичную матрицу, как нулевой поворот $R_0 = I$, а затем последовательно попорачивать базисные вектора вокруг нужных нам осей. В результате получим матрицу поворота соотвектвующую углам Эйлера. Например:  
...
Также можно отдельно вывести матрицы вращения по каждой из осей $X$, $Y$, $Z$ ($R_x$, $R_Y$, $R_z$ соотвественно) и получить итоговую матрицу последовательным их умножением:
$R = R_x \cdot R_y \cdot R_z$

Итак, с вращением объекта разобрлись. Переходим к остальным трансформациям.  
...
До этого момента точка начала локальных координат не смещалась в мировом пространсве. Так как точка начала координат нашего объекта - это его центр, то центр объект никуда не смещался. Реализовать это смещение просто: $\vec{v'_i} = \vec{v_i} + \vec t$, где $\vec t$ - вектор, задающий смещение, а в нашем случае указывающий на сколько сметиться центр объекта.  
Теперь мы умеем масштабировать объект по осям, поворачивать его и перемещать. Объеденим все одной формулой: $\vec{v'_i} = R \cdot S \cdot \vec{v_i} + \vec t$:

![s_x = 0.5, \space s_y = 0.5, \space $\alpha={30^{\circ}}, \space \vec t = \begin{pmatrix} 6 & 4 \end{pmatrix}^T$](./assets/Figure_10.png)

Чтобы упростить формулу, мы можем, как уже делали ранее, объединить матрицы $T = R \cdot S \space \Rightarrow \space \vec{v'_i} = T \cdot \vec{v_i} + \vec t$. В итоге наше преобразование описывает матрица $T$ и вектор $\vec t$. Объединение вектора $\vec t$ с матрицей $T$ еще более бы упростило формулу, однако сделать в даннном случае не получится, потому как сложение здесь - это не линейная операция. Тем не менее сделать это возможно, и рассмотрим этот момент уже в следующей статье.

## Заключение.  

Для какого покажется, что статья описывет очевидные вещи, кому-то может показаться наоборот немного запутанной. Тем не менее это базовый фундамент, на котором будет строиться все остальное. Векторная алгебра - является фундаментом для многих областей, так что статья может вам оказаться полезной не только дополненной реальности. Следующая статья будет уже более узконаправленной.