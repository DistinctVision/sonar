...
Получается координаты спроецированной на плоскость точки: $\vec{p} = \begin{pmatrix} \frac{v_x}{v_z} & \frac{v_y}{v_z}\end{pmatrix}$. Ничего не напоминает? Это формула перевода из однородных координат в двумерные эвклидовы. Получается, что при таких параметрах плоскости проецирования, трехмерные координаты можно воспринимать как однородные координаты, соответствущие двумерным эвклидовым координатам на плоскости. А перевод из однородных координат в двумерные эвклидовы координаты выполняет центральную проекцию. Можно сказать, что трехмерные эвклидово пространство эквивалентно двумерному проекционному в данном случае.
...
$x' = x \cdot \frac{width}{\tan{\frac{angle_x}{2}}}$  
Если мы сохранить пропорции изображения, то координату $y$ мы должны масштабировать с тем же коэффициентом. Получаем такое преобразование:  
$f = \frac{width}{\tan{\frac{angle_x}{2}}}, \space \vec{p'} = \vec{p} \cdot f$.  
...
$\vec{p'} = \vec{p} \cdot f + \begin{pmatrix} \frac{width}{2} & \frac{height}{2} \end{pmatrix}^T$   
...
$\vec{p'} = \vec{p} \cdot f + \vec{c}$, где $f = \frac{width}{\tan{\frac{angle_x}{2}}}$, а $\vec{c} = \begin{pmatrix} \frac{width}{2} & \frac{height}{2} \end{pmatrix}^T$.  
...
Пусть $K = \begin{pmatrix} f & 0 & c_x \\ 0 & f & c_y \\ 0 & 0 & 1 \end{pmatrix}$, тогда $\vec{p'} = project(K \cdot \begin{pmatrix} p_x \\ p_y \\ 1\end{pmatrix})$.  
...
Мы рассмотрели формирование этой матрицы в несколько упрощенной (пусть сейчас и достаточной для нас) форме. В общем случае эта матрица имеет такую форму:  
$K = \begin{pmatrix} f_x & s_{skew} & c_x \\ 0 & f_y & c_y \\ 0 & 0 & 1 \end{pmatrix}$.  
В нашем случае $f_x = f_y$, потому что нам сохранить пропорции изображения. А формулы вычисления параметров $f, c_x, c_y$ справедливы, если мы хотим перевести именно в пиксельные координаты изображения. Но других случаях нам может быть нужно перевести в другую координатную систему, например, в текстурные координаты, где значения координат $x$ и $y$ меняются в диапазоне $[0, 1]$. Тогда эти параметрами будут уже другими.  
Здесь есть еще дополнительный параметр $s_{skew}$, отвечающий за скос изображения. Эффект от его применения чем схож эффектом от [rolling shutter](https://ru.wikipedia.org/wiki/%D0%A0%D0%BE%D0%BB%D0%BB%D0%B8%D0%BD%D0%B3-%D1%88%D0%B0%D1%82%D1%82%D0%B5%D1%80). На практике обычно этот параметр не учитывается и остается равно нулю.  
...
Сейчас же предлагаю просто считать оптическим центром изображения и подбирать параметр $f$ исходя из взятого на глаз угла обзора по горизонтали. Для начала хватит и такого варианта.
...
Параметры, отвечающие за положение камеры в мировом пространстве $R$ и $\vec{t}$, называют внешними параметрами камеры (extrinsic camera parameters).

И к этому моменту можно нас поздравить, мы получили представление о внутренних и внешних параметрах камеры, а самое главное - построили математическую модель построения изображения камеры - та самая последняя формула выше. 